...
| **窗口模式切换（悬浮窗→全屏）**  | ✅ 设置弹窗自动关闭，阅读器以全屏模式立即重开（无置顶按钮/无拖动手柄），恢复原页面（目录页）                                                                      |
| **窗口模式切换（全屏→悬浮窗）**  | ✅ 设置弹窗自动关闭，置顶按钮 + 拖动手柄重新出现（悬浮窗专属 UI），恢复原页面（目录页）                                                                           |
| **设置持久化**                   | ✅ 重开设置弹窗时下拉框正确显示已选模式                                                                                                                           |
| **窗口模式切换后控制台**         | ✅ 无插件相关错误                                                                                                                                                 |

### 8.3 语义修正（v3，2026-09-26）：仅"阅读器打开期间"的生成结束才通知

**用户反馈的边界问题**：

- ✅ 用户发送信息 → AI 开始生成 → **尚未生成完毕**时打开阅读器 → 生成结束瞬间阅读器打开 → 红点/气泡**正常**
- ❌ 用户发送信息 → AI **生成完毕**（用户已在酒馆看到完整楼层）→ 之后打开阅读器 → 红点/气泡**不应出现**

**原因**：v2 为修复"先发送再打开弹窗"漏通知，采用**启动时常驻订阅 + 后台无条件累计**（`pending += 1`），导致阅读器关闭期间发生的生成结束也被累计，打开时 `getPending()` 立即显示 → 用户已看过楼层仍被提醒。

**v3 修正**：通知语义改为**"生成结束的那一刻阅读器弹窗必须处于打开状态"**：

1. **`features/gen-notify/index.js`**：新增依赖注入 `isReaderOpen`；`bump()` 入口加判定 `if (!deps.isReaderOpen()) return;` —— 阅读器未打开时不累计、不回调（用户正看着酒馆，楼层已完整可见，无需提醒）。
2. **`index.js`**：
   - 实例化 `createGenNotifyCore` 时注入 `isReaderOpen: () => dialogRef !== null`；
   - `bindGenNotifyUi` 移除"打开时 `genNotify.getPending() > 0` 立即显示"逻辑（v2 后台累计语义已废除，关闭期间不会再有 pending）；
   - `dlg.onClose` 注释同步更新（订阅仍常驻，但通知以生成结束时刻阅读器是否打开为准）。
3. **设置项提示文案**：更新为"阅读器打开期间，若酒馆有新楼层生成结束或被截断…生成结束时阅读器未打开（你能看到酒馆楼层）则不会提醒"。

**时序判定说明**：

- 生成**进行中**（`GENERATION_STARTED` 已触发、`ENDED/STOPPED` 未触发）时用户打开阅读器 → 之后 `ENDED/STOPPED` 到来时 `isReaderOpen()` 为 true → **通知**；
- 生成**已结束**后才打开阅读器 → `ENDED/STOPPED` 早已过去，`isReaderOpen()` 当时为 false → **不累计、不通知**。

**与 v2 常驻订阅的关系**：订阅仍在插件启动时建立并常驻（幂等，`subscribe()` 内部先 `unsubscribe()`），保证生成结束事件**不会漏听**；`isReaderOpen` 判定在 `bump()` 处统一拦截，阅读器关闭期间的事件被丢弃而非累计。

#### 8.3.1 浏览器回归测试结果（v3，全部通过）

| 场景                              | 结果                                                        |
| --------------------------------- | ----------------------------------------------------------- |
| **生成中打开阅读器 → 生成结束**   | ✅ 红点闪烁 + 气泡"有新楼层生成"（生成结束时刻阅读器打开）   |
| **生成完毕后再打开阅读器**        | ✅ 无红点/气泡（生成结束时刻阅读器未打开，事件被丢弃）       |
| **手动停止（STOPPED）生成中打开** | ✅ 触发通知（同 v1/v2 行为）                                 |
| **阅读器关闭期间生成结束**        | ✅ 不累计、不通知（`isReaderOpen()` false）                  |
| **重新打开阅读器**                | ✅ 无残留红点（关闭时已 `clear()`，且关闭期间不累计）        |
| **开关关闭时生成结束**            | ✅ 不触发（订阅移除，事件不进入状态机）                      |
| **订阅幂等性（回归）**            | ✅ 常驻订阅 + `unsubscribe()` 先取消再订阅，监听器数量不变   |
| **设置项文案**                    | ✅ 已更新为"阅读器打开期间…生成结束时阅读器未打开则不会提醒" |
